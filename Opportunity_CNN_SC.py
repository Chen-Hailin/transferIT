...
with graph.as_default():
    ...
    with tf.variable_scope("origin"):
        weights12 = tf.get_variable("weight12", shape=[1, 5, 1, conv_multiplier], dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        biases12 = tf.get_variable("biase12", shape=[conv_multiplier], dtype=tf.float32,
                                   initializer=tf.constant_initializer(0))
        weights23 = tf.get_variable("weight23", shape=[1, 5, conv_multiplier, conv_multiplier], dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        biases23 = tf.get_variable("biase23", shape=[conv_multiplier], dtype=tf.float32,
                                   initializer=tf.constant_initializer(0))
        weights34 = tf.get_variable("weight34", shape=[1, 5, conv_multiplier, conv_multiplier], dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        biases34 = tf.get_variable("biase34", shape=[conv_multiplier], dtype=tf.float32,
                                   initializer=tf.constant_initializer(0))
        weights45 = tf.get_variable("weight45", shape=[1, 5, conv_multiplier, conv_multiplier], dtype=tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        biases45 = tf.get_variable("biase45", shape=[conv_multiplier], dtype=tf.float32,
                                   initializer=tf.constant_initializer(0))
        last_weights = tf.get_variable("weightl", shape=[n_hidden, n_classes], dtype=tf.float32,
                                       initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        last_biases = tf.get_variable("biasel", shape=[n_classes], dtype=tf.float32,
                                      initializer=tf.constant_initializer(0))

        tf.get_variable_scope().reuse_variables()

    # layer2 = apply_conv(x_, 5, 1, conv_multiplier)
    layer2 = tf.nn.relu(tf.add(tf.nn.conv2d(x_, weights12, strides=[1, 1, 1, 1], padding='VALID'), biases12))
    #layer3 = apply_conv(layer2, 5, conv_multiplier, conv_multiplier)
    layer3 = tf.nn.relu(tf.add(tf.nn.conv2d(layer2, weights23, strides=[1, 1, 1, 1], padding='VALID'), biases23))
    # layer4 = apply_conv(layer3, 5, conv_multiplier, conv_multiplier)
    layer4 = tf.nn.relu(tf.add(tf.nn.conv2d(layer3, weights34, strides=[1, 1, 1, 1], padding='VALID'), biases34))
    # layer5 = apply_conv(layer4, 5, conv_multiplier, conv_multiplier)
    layer5 = tf.nn.relu(tf.add(tf.nn.conv2d(layer4, weights45, strides=[1, 1, 1, 1], padding='VALID'), biases45))
    ...
    # saver = tf.train.Saver({"weight12": weights12, "weight23": weights23, "weight34": weights34, "weight45": weights45, "biase12": biases12, "biase23": biases23, "biase34": biases34, "biase45": biases45})
...
with graph2.as_default():
    ...
    with tf.variable_scope("origin"):
        weights12_t = tf.get_variable("weight12", shape=[1, 5, 1, conv_multiplier])
        biases12_t = tf.get_variable("biase12", shape=[conv_multiplier])
        weights23_t = tf.get_variable("weight23", shape=[1, 5, conv_multiplier, conv_multiplier])
        biases23_t = tf.get_variable("biase23", shape=[conv_multiplier])
        weights34_t = tf.get_variable("weight34", shape=[1, 5, conv_multiplier, conv_multiplier])
        biases34_t = tf.get_variable("biase34", shape=[conv_multiplier])
        weights45_t = tf.get_variable("weight45", shape=[1, 5, conv_multiplier, conv_multiplier])
        biases45_t = tf.get_variable("biase45", shape=[conv_multiplier])
        last_weights_t = tf.get_variable("weightl", shape=[n_hidden, n_classes], dtype=tf.float32,
                                       initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        last_biases_t = tf.get_variable("biasel", shape=[n_classes], dtype=tf.float32,
                                      initializer=tf.constant_initializer(0))
    ...
    layer4_t = tf.nn.relu(tf.add(tf.nn.conv2d(layer_stop_grad_2, weights34_t, strides=[1, 1, 1, 1], padding='VALID'), biases34_t))
    # layer5 = apply_conv(layer4, 5, conv_multiplier, conv_multiplier)

    # layer_stop_grad_3 = tf.stop_gradient(layer4_t)
    ...
    lossL2_t = tf.add_n([tf.nn.l2_loss(v) for v in all_vars_t if 'bias' not in v.name]) * 0.0005

    loss_t = tf.add(cross_entropy_transfer, lossL2_t)

    global_step_t = tf.Variable(0, trainable=False)
    train_op = tf.train.RMSPropOptimizer(learning_rate_t).minimize(loss_t, global_step=global_step_t)
    ...
    # saver_t = tf.train.Saver({"weight12": weights12_t, "weight23": weights23_t, "weight34": weights34_t, "weight45": weights45_t, "biase12": biases12_t, "biase23": biases23_t, "biase34": biases34_t, "biase45": biases45_t})
...
def iterate_transfer_batches(inputs, targets, batchsize, shuffle=False):
    print("len " + str(len(inputs)) + " " + str(len(targets)) + " ")
    assert len(inputs) == len(targets)
    assert len(inputs) > batchsize
    if shuffle:
        indices = np.arange(len(inputs))
        np.random.shuffle(indices)
    for start_idx in range(0, len(inputs) - batchsize + 1, batchsize / 2):
        if shuffle:
            excerpt = indices[start_idx:start_idx + batchsize]
        else:
            excerpt = slice(start_idx, start_idx + batchsize)
...
def main():
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options), graph=graph) as sess:
        ...
        for iteration in range(181):
            ...
            if (iteration % 10 == 0):
                ...
                print("Iter " + str(iteration) + " Subject" + str(source_sub) + " Train F1 score= " + "{:.5f}".format(f1))
                test_pred = np.empty((0))
                test_true = np.empty((0))
            # tf_confusion_metrics(result, y_train[start:], sess, {x : X_train[start:]})
            if (iteration % 20 == 0):
    ...
    with tf.Session(graph=graph2, config=tf.ConfigProto(gpu_options=gpu_options_t)) as session_transfer:
        ...
        #change here
        saver_t.restore(session_transfer, "/tmp/model.ckpt")

        print("Session 2: Model restored")

        results = []
        for iteration in range(451):
            # start_random = random.randint(0, batch_size)
            # start = start_random
            step = 0
            for batch in iterate_minibatches(x_train_t, y_train_t, batch_size, shuffle=True):
                batch_x, batch_y = batch
                # session_transfer.run(train_op, feed_dict={x_t: batch_x, y_t: batch_y, keep_prob_t: 0.5})
                acc = session_transfer.run(accuracy_transfer, feed_dict={x_t: batch_x, y_t: batch_y, keep_prob_t: 1.0})
                if (acc < 0.7 and iteration > 20):
                    session_transfer.run(train_op, feed_dict={x_t: batch_x, y_t: batch_y, keep_prob_t: 0.5})
                if (acc < 0.8 + 0.0015 * iteration):
                    session_transfer.run(train_op, feed_dict={x_t: batch_x, y_t: batch_y, keep_prob_t: 0.5})
            ...
            test_true = np.empty((0))
            # tf_confusion_metrics(result, y_train[start:], sess, {x : X_train[start:]})
            for batch in iterate_minibatches(x_train_t, y_train_t, batch_size):
                inputs, targets = batch
                ...
                test_true = np.append(test_true, np.argmax(targets, axis=1), axis=0)
            f1 = sk.metrics.f1_score(test_true, test_pred, average="weighted")
            print("Iter " + str(iteration) + " Subject" + str(target_sub) + " Train F1 score= " + "{:.5f}".format(f1))
            ...
            if (f1 > 0.5):
                ...
                print(f1)
        filename = os.path.join(result_path, "ft" + str(source_sub) + str(target_sub) + ".csv")
        with open(filename, "w") as fout:
            fout.write("Iteration,f1 score\n")
            for i in range(len(results)):
                fout.write(str(i+1) + "," + str(results[i]) + "\n")
